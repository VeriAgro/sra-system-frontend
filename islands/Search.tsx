  ...
  const fetchData = async (searchId: string) => {
    try {
      const [animalResponse, eventsResponse] = await Promise.all([
        fetch(
          `https://sra-system-backend-999646529726.southamerica-east1.run.app/animal/${searchId}`
        ),
        fetch(
          `https://sra-system-backend-999646529726.southamerica-east1.run.app/event/animal/${searchId}`
        )
      ]);
